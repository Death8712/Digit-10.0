      ...
      <div className="space-y-12 relative pb-12">
        {timelineData.map((item, index) => (
          <div key={item.year} className={`relative flex flex-col md:flex-row items-start md:items-center justify-between gap-8 group reveal ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
            ...
            {/* Content Box */}
            <div className={`w-full md:w-5/12 ml-[50px] md:ml-0 glass p-6 rounded-2xl border-white/5 border group-hover:border-neon-cyan/30 transition-colors shadow-lg hover:shadow-[0_0_20px_rgba(0,255,255,0.1)]`}> 
